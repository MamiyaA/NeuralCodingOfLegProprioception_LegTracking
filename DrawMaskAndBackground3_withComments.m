...
%tibia angle.
%
%StartT1: Assuming that the video captures a ramp-and-hold trial, StartT1
...
%
%StartT2: Assuming that the video captures a ramp-and-hold trial, StartT2
...
for n=1:FramesToRead
    ...
    AllImage(:,:,n)=readFrame(ExampleVid);
end
...
k=1;
while k<=FrameN1
    AllImage2(:,:,k)=readFrame(ExampleVid);
    k=k+1;
end
...
%To find the backgroun for a region initially covered by the tibia, go to a
...
k=1;
while k<=FrameN2
    AllImage3(:,:,k)=readFrame(ExampleVid);
    k=k+1;
end
